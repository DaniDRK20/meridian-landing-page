...
export function WorkspaceShell({ user, children }: { user: { id: string; name: string; email: string }; children: ReactNode }) {
  ...
  useEffect(()=>{let stopped=false;const refresh=()=>fetch("/api/admin/chat?summary=1",{cache:"no-store"}).then(response=>response.json()).then(result=>{if(stopped||!result.ok)return;const next=Number(result.unread)||0,previous=unreadRef.current;setUnread(next);unreadRef.current=next;if(pathname!=="/admin/chat"&&previous!==null&&next>previous&&result.latest&&result.latest.id!==latestRef.current){latestRef.current=result.latest.id;const mentioned=Boolean(result.latest.mentioned),title=mentioned?`${result.latest.author_name} te mencionó`:`Nuevo mensaje de ${result.latest.author_name}`;setToast({title,body:result.latest.content,mentioned});window.setTimeout(()=>setToast(null),6000)}}).catch(()=>undefined);void refresh();const timer=window.setInterval(refresh,realtime?12000:4000);return()=>{stopped=true;window.clearInterval(timer)}},[pathname,realtime]);
